...
#include <ElasticMembranePlateSectionUncoupled.h> // CSUF
	...
	TCL_Char** argv, TclModelBuilder* theTclBuilder)
{
	...
	//--- Adding Thermo-mechanical Sections:[BEGIN]   by UoE OpenSees Group ---//  
	...
	else if (strcmp(argv[1], "ElasticMembranePlateSectionUncoupled") == 0) { // CSUF
		if (argc < 7) {
			opserr << "WARNING insufficient arguments\n";
			opserr << "Want: section ElasticMembranePlateSectionUncoupled tag? E? nu? h? tbend? tmembrane? <rho?>" << endln;
			return TCL_ERROR;
		}

		int tag;
		double E, nu, h, tbend, tmembrane;
		double rho = 0.0;

		if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
			opserr << "WARNING invalid section ElasticMembranePlateSectionUncoupled tag" << endln;
			return TCL_ERROR;
		}

		if (Tcl_GetDouble(interp, argv[3], &E) != TCL_OK) {
			opserr << "WARNING invalid E" << endln;
			opserr << "ElasticMembranePlateSectionUncoupled section: " << tag << endln;
			return TCL_ERROR;
		}
		...
		if (Tcl_GetDouble(interp, argv[6], &tbend) != TCL_OK) {
			opserr << "WARNING invalid tbend" << endln;
			opserr << "ElasticMembranePlateSectionUncoupled section: " << tag << endln;
			return TCL_ERROR;
		}

		if (Tcl_GetDouble(interp, argv[7], &tmembrane) != TCL_OK) {
			opserr << "WARNING invalid tmembrane" << endln;
			opserr << "ElasticMembranePlateSectionUncoupled section: " << tag << endln;
			return TCL_ERROR;
		}
		...
		theSection = new ElasticMembranePlateSectionUncoupled(tag, E, nu, h, rho, tbend, tmembrane);
	}
