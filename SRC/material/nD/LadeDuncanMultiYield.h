...
#ifndef LadeDuncanMultiYield_h
#define LadeDuncanMultiYield_h

#include <NDMaterial.h>
#include <MultiYieldSurface.h> 
#include <Matrix.h>
using namespace std;
class LadeDuncanMultiYield : public NDMaterial
{
public:
     // Initialization constructor
     LadeDuncanMultiYield (int tag,
			       int nd,
				   double rho,
			       ...
			       double atm = 101.,
				   double cohesi = 0.1, double dilationParam4 = 0.0,
					double contractionParam4 = 0.0,
					double contractionParam5 = 0.0,
					double liquefactionParam3 = 0.0,
				   double hv = 0.,
				   double pv = 1.);

     // Default constructor
     LadeDuncanMultiYield ();

     // Copy constructor
     LadeDuncanMultiYield (const LadeDuncanMultiYield &);

     // Destructor: clean up memory storage space.
     virtual ~LadeDuncanMultiYield ();
	 ...
	 const Vector &getCommittedCenter(void);
	 const double &getCommittedPPZSize(void);
	 const Vector &getCommittedPivot(void);
     ...
     // Return a copy of itself if "code"="LadeDuncanMultiYield", otherwise return null.
     NDMaterial *getCopy (const char *code);

     // Return the string "LadeDuncanMultiYield".
...
private:
  // user supplied
	 ...
	 double maxPress;
	 double Dilation_Rule;
	 double Dilation_Rule1;
	 ...
	 double TRIPRO (Vector &A, Vector &B, Vector &C,int I);  // result = A:B:C
     double stressRatio(const Vector & tdev, double tdil);
	 double thirdOrderEqn(double A, double B, double C, double D, double I);

	 double ContractionFactorC(void); 
	 Vector stress0; 
	 bool stress0Initialized; 
};
