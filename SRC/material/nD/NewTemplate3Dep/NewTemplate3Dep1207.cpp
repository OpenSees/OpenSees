...
int NewTemplate3Dep::revertToStart(void)
{
    ...
    TrialStrain = CommitStrain = ZeroStrain;
...
int NewTemplate3Dep::ForwardEuler(const straintensor& strain_incr)
{
    ...
    start_strain = this->getStrainTensor();
    start_stress = this->getStressTensor();
    ...
    //cout << "f1 = " << f_start << " " << "f2 = " << f_pred << endl;    
    
    // If Elastic
    if ( (f_start <= 0.0 && f_pred <= 0.0) || f_start > f_pred ) {
        ...
        Ee_updated = pointer_elastic_state->getElasticStiffness(*pointer_material_parameter);
        Stiffness = (Ee + Ee_updated) *0.5;
    ...
    if ( f_start < 0.0 )  {
        straintensor strain_incr_to_intersection;
        stresstensor stress_incr_to_intersection;
              
        Intersection_stress = yield_surface_cross( start_stress, elastic_predictor_stress );
        ...
        Ee_updated = pointer_elastic_state->getElasticStiffness(*pointer_material_parameter);
        Ee = (Ee + Ee_updated) *0.5;
        Ee_inv = Ee.inverse();
        stress_incr_to_intersection = Intersection_stress - start_stress;
        strain_incr_to_intersection = Ee_inv("ijpq") * stress_incr_to_intersection("pq");
        stress_incr_to_intersection.null_indices();
        Intersection_strain = start_strain + strain_incr_to_intersection;    
