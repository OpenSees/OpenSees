...
{	
  ...
  if (de > 0) {
	...
	if (newStrain <= etint) {
	  newStressPure = (fct - ftint)/(et0 - etint)*(newStrain - etint) + ftint;
	  newStress = newStressPure;
	  newTangent = (newStress - Tstress)/(newStrain - Tstrain);
	} else if (newStrain <= etres) {
	  newStressPure = (ftint - ftres)/(etint - etres)*(newStrain - etres) + ftres;
	  newStress = newStressPure;
	  newTangent = (newStress - Tstress)/(newStrain - Tstrain);
	} else {
	  newStressPure = ftres;
	...
	double SecTang = (CMaxStressTens)/(CMaxStrainTens - zeroStrain);
	newStressPure = SecTang*(newStrain - zeroStrain);
	newStress = newStressPure;
	newTangent = (newStress - Tstress)/(newStrain - Tstrain);
      }
    } else {
	...
	newStressPure = E0*newStrain + (fpc/(ec0*ec0)-E0/ec0)*(newStrain*newStrain);
	...
	newStressPure = (fpc-fcc)/pow(ec0-ecc,3)*pow(newStrain-ecc,3)+fcc;
