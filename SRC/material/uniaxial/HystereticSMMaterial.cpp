...
HystereticSMMaterial::setResponse(const char** argv, int argc, OPS_Stream& theOutput)
{
    ...
    if (strcmp(argv[0], "defoPlasticMax") == 0) {
        return new MaterialResponse(this, 211, Vector(3));
    }
...
HystereticSMMaterial::getResponse(int responseID, Information& matInfo)
{
    ...
    else if (responseID == 211) {
        static Vector data(3);

        data(0) = this->CrotNu - rot1n;
        data(1) = this->CrotPu - rot1p;

        double thetaPn = 0;
        double thetaPp = 0;
        if (CrotNu < rot1n) {
            thetaPn = fabs((CrotNu - rot1n));
        }
        if (CrotPu > rot1p) {
            thetaPp = fabs((CrotPu - rot1p));
        }
        if (thetaPp > thetaPn) {
            data(2) = thetaPp;
        }
