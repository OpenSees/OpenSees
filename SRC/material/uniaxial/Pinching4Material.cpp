...
Pinching4Material::Pinching4Material(int tag,
				     ...
				     double gf1, double gf2, double gf3, double gf4, double gflim, double ge):
...
/*	if ( DEBG ==1 )
    {
      // Open an output file for debugging
		...
		fn = new FileStream();
		fn->setFile("Pinching4Material.out" , OVERWRITE);
		//(*fn) << "This sucks!" << endln;
    }
*/  
  ...
  rDispP(mdp), rForceP(mfp), uForceP(msp)
{
	...
	if ( DEBG ==1 )
    {
      // Open an output file for debugging
//     fn = FileStream();
//	  fn.setFile("Pinching4Material.out" , APPEND);
//	  fn << "This sucks!" << endln;
    }
...
int Pinching4Material::setTrialStrain(double strain, double CstrainRate)
{
	...
	case 2:
		...
		break;
	case 3:
		kunload = (hghTstateStrain<0) ? kElasticNegDamgd:kElasticPosDamgd; 	
		...
		break;
	case 4:
		kunload = (lowTstateStrain<0) ? kElasticNegDamgd:kElasticPosDamgd;
	...
	elasticStrainEnergy = (Tstrain>0) ? 0.5*Tstress/kElasticPosDamgd*Tstress:0.5*Tstress/kElasticNegDamgd*Tstress;
...
// ******************************************************	
	...
	for (int i=0; i<=5; i++) {
		envlpPosDamgdStress[i] = envlpPosStress[i]*(1-gammaFUsed);
		envlpNegDamgdStress[i] = envlpNegStress[i]*(1-gammaFUsed);
	}

// *******************************************************

//	(*fn) << Cstrain << "  " << Cstress << "  "  << Cstate << endln;
	return 0;
}
...
int Pinching4Material::revertToStart(void)
{
	...
	for (int j = 0; j<6; j++)
	{
		envlpPosDamgdStress[j] = envlpPosStress[j];
		envlpNegDamgdStress[j] = envlpNegStress[j];
	}
...
//	s << "tangent: " << Ttangent << endln;
	s << "state: " << Tstate << endln;
/*	if (Tstate == 3 )
	{
		s << "1st pnt: " << state3Stress[0]  << "  " << state3Strain[0] << endln;
		s << "2nd pnt: " << state3Stress[1]  << "  " << state3Strain[1] << endln;
		s << "3rd pnt: " << state3Stress[2]  << "  " << state3Strain[2] << endln;
		s << "4th pnt: " << state3Stress[3]  << "  " << state3Strain[3] << endln;
		s << "gamma s: " << TgammaK << "  " << TgammaD << "  " << TgammaF << endln;
	}
	if (Tstate ==4)
	{
		s << "1st pnt: " << state4Stress[0]  << "  " << state4Strain[0] << endln;
		s << "2nd pnt: " << state4Stress[1]  << "  " << state4Strain[1] << endln;
		s << "3rd pnt: " << state4Stress[2]  << "  " << state4Strain[2] << endln;
		s << "4th pnt: " << state4Stress[3]  << "  " << state4Strain[3] << endln;
		s << "gamma s: " << TgammaK << "  " << TgammaD << "  " << TgammaF << endln;
	}
*/
}
...
{ 
	...
	envlpPosStress[5] = (k1>0)? stress4p+k1*(envlpPosStrain[5] - strain4p):stress4p*1.1;
	envlpNegStrain[5] = 1e+6*strain4n;
	envlpNegStress[5] = (k2>0)? stress4n+k2*(envlpNegStrain[5] - strain4n):stress4n*1.1;
...
//					Print(opserr,1);
					...
					hghTstateStress = posEnvlpStress(uMaxDamgd);
					gammaKUsed = CgammaK;      // from T to C
					kElasticNegDamgd = kElasticNeg*(1.0-gammaKUsed); 
				}
			}
			else if (Tstate == 4){
...
void Pinching4Material::getState3(double* state3Strain, double* state3Stress, double kunload)
		{

			double kmax = (kunload>kElasticNegDamgd) ? kunload:kElasticNegDamgd;

			if (state3Strain[0]*state3Strain[3] <0){
				...
				if (state3Strain[1]>state3Strain[3]) {
					...
					if (state3Strain[2] > state3Strain[3]) {
						...
						state3Stress[2] = state3Strain[1] + 0.5*df;
					}
				...
				while (i<3) {
					...
					if (slope > 1e-8 && slope < checkSlope) {
						...
						state3Strain[2] = state3Strain[3]/2; state3Stress[2] = state3Strain[3]/2;
					} 
					i++;
				}
...
void Pinching4Material::getState4(double* state4Strain,double* state4Stress, double kunload)
		{

			double kmax = (kunload>kElasticPosDamgd) ? kunload:kElasticPosDamgd;

			if (state4Strain[0]*state4Strain[3] <0){
				...
				if (state4Strain[2]<state4Strain[0]) {
					...
					if (state4Strain[1] < state4Strain[0]) {
						...
						state4Stress[1] = state4Strain[0] + 0.5*df;
					}
				...
				while (i<3) {
					...
					if (slope > 1e-8 && slope < checkSlope) {
						...
						state4Strain[2] = state4Strain[3]/2; state4Stress[2] = state4Strain[3]/2;
					} 
...
double Pinching4Material::Envlp3Tangent(double* state3Strain, double* state3Stress, double u)
			{
				double k = 0;
				int i = 0;
				while ((k==0||i<=2) && (i<=2)) 
				{
					if (u>= state3Strain[i]) {
						k = (state3Stress[i+1]-state3Stress[i])/(state3Strain[i+1]-state3Strain[i]);
					}
					i++;
				}
				if (k==0) {
...
double Pinching4Material::Envlp4Tangent(double* state4Strain, double* state4Stress, double u)
			{
				double k = 0;
				int i = 0;
				while ((k==0||i<=2) && (i<=2)) 
				{
					if (u>= state4Strain[i]) {
						k = (state4Stress[i+1]-state4Stress[i])/(state4Strain[i+1]-state4Strain[i]);
					}
					i++;
				}
				if (k==0) {
  ...
  double Pinching4Material::Envlp3Stress(double* state3Strain, double* state3Stress, double u)
			{
				double k = 0.0;
				int i = 0;
				double f = 0.0;
				while ((k==0||i<=2) && (i<=2)) 
				{
					if (u>= state3Strain[i]) {
						k = (state3Stress[i+1]-state3Stress[i])/(state3Strain[i+1]-state3Strain[i]);
						f = state3Stress[i]+(u-state3Strain[i])*k;
					}
					i++;
				}
				if (k==0) {
...
double Pinching4Material::Envlp4Stress(double* state4Strain, double* state4Stress, double u)
			{
				double k = 0.0;
				int i = 0;
				double f = 0.0;
				while ((k==0||i<=2) && (i<=2)) 
				{
					if (u>= state4Strain[i]) {
						k = (state4Stress[i+1]-state4Stress[i])/(state4Strain[i+1]-state4Strain[i]);
						f = state4Stress[i]+(u-state4Strain[i])*k;
					}
					i++;
				}
				if (k==0) {
...
void Pinching4Material::updateDmg(double strain)
	{
		...
		if ((strain<uultAbs && strain>-uultAbs)&& Tenergy< energyCapacity)
		{
			...
			double kmin = (kminP>kminN) ? kminP:kminN;
			double ke = (kElasticPos<kElasticNeg) ? kElasticPos:kElasticNeg;
			double gammaKLimEnv = (0.0>(1.0-kmin/ke)) ? 0.0:(1.0-kmin/ke);
