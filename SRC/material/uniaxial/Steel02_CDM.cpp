...
OPS_Steel02_CDM()
{
  ...
  if (OPS_GetIntInput(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial Steel02_CDM tag" << endln;
    return 0;
  }
  ...
  if (numData != 3 && numData != 6 && numData != 10 && numData != 11) {
    opserr << "Invalid #args, want: uniaxialMaterial Steel02_CDM " << iData[0] << 
  ...
  if (numData == 3) {
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
      opserr << "Invalid double: uniaxialMaterial Steel02_CDM " << iData[0] << 
	" fy? E? b? <R0? cR1? cR2? <a1? a2? a3? a4?>>" << endln;
      return 0;
    }

    // Parsing was successful, allocate the material
    theMaterial = new Steel02_CDM(iData[0], dData[0], dData[1], dData[2]);    

  } else if (numData == 6) {
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
      opserr << "Invalid int: uniaxialMaterial Steel02_CDM " << iData[0] << 
	" fy? E? b? <R0? cR1? cR2? <a1? a2? a3? a4?>>" << endln;
      return 0;
    }

    // Parsing was successful, allocate the material
    theMaterial = new Steel02_CDM(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5]);    

  } else if (numData == 10) {
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
      opserr << "Invalid arggs: uniaxialMaterial Steel02_CDM " << iData[0] << 
	" fy? E? b? <R0? cR1? cR2? <a1? a2? a3? a4?>>" << endln;
      return 0;
    }
  ...
  if (theMaterial == 0) {
    opserr << "WARNING could not create uniaxialMaterial of type Steel02_CDM Material\n";
    return 0;
  }
...
Steel02_CDM::Steel02_CDM(int tag,
  ...
  UniaxialMaterial(tag, MAT_TAG_Steel02_CDM),
...
Steel02_CDM::Steel02_CDM(int tag, double _Fy, double _E0, double _b):
  UniaxialMaterial(tag, MAT_TAG_Steel02_CDM),
...
Steel02_CDM::Steel02_CDM(void):
  UniaxialMaterial(0, MAT_TAG_Steel02_CDM)
{
	EnergyP = 0;	//by SAJalali
	konP = 0;
}
...
UniaxialMaterial*
Steel02_CDM::getCopy(void)
{
  Steel02_CDM *theCopy = new Steel02_CDM(this->getTag(), Fy, E0, b, R0, cR1, cR2, a1, a2, a3, a4, sigini);
  
  return theCopy;
}

double
Steel02_CDM::getInitialTangent(void)
{
  return E0;
}

int
Steel02_CDM::setTrialStrain(double trialStrain, double strainRate)
{
  double Esh = b * E0;
  double epsy = Fy / E0;
...
double 
Steel02_CDM::getStrain(void)
{
  return eps;
}

double 
Steel02_CDM::getStress(void)
{
  return sig;
}

double 
Steel02_CDM::getTangent(void)
{
  return e;
}

int 
Steel02_CDM::commitState(void)
{
  epsminP = epsmin;
  epsmaxP = epsmax;
  epsplP = epspl;
...
int 
Steel02_CDM::revertToLastCommit(void)
{
  epsmin = epsminP;
  epsmax = epsmaxP;
  epspl = epsplP;
...
int 
Steel02_CDM::revertToStart(void)
{
	EnergyP = 0;	//by SAJalali
	eP = E0;
  epsP = 0.0;
...
int 
Steel02_CDM::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.sendVector(this->getDbTag(), commitTag, data) < 0) {
    opserr << "Steel02_CDM::sendSelf() - failed to sendSelf\n";
    return -1;
  }
  return 0;
}
...
int 
Steel02_CDM::recvSelf(int commitTag, Channel &theChannel, 
	     FEM_ObjectBroker &theBroker)
{
  static Vector data(23);

  if (theChannel.recvVector(this->getDbTag(), commitTag, data) < 0) {
    opserr << "Steel02_CDM::recvSelf() - failed to recvSelf\n";
    return -1;
  }
...
void 
Steel02_CDM::Print(OPS_Stream &s, int flag)
{
  if (flag == OPS_PRINT_PRINTMODEL_MATERIAL) {      
    //    s << "Steel02_CDM:(strain, stress, tangent) " << eps << " " << sig << " " << e << endln;
    s << "Steel02_CDM tag: " << this->getTag() << endln;
    s << "  fy: " << Fy << ", ";
    s << "  E0: " << E0 << ", ";
    s << "   b: " << b << ", ";
    s << "  R0: " << R0 << ", ";
...
int
Steel02_CDM::setParameter(const char **argv, int argc, Parameter &param)
{
...
int
Steel02_CDM::updateParameter(int parameterID, Information &info)
{
  switch (parameterID) {
  case -1:
    return -1;
