...
OPS_Export void *OPS_ConcreteCM(void)
{
	...
	if (OPS_GetIntInput(&numData, iData) != 0) {
		opserr << "WARNING invalid tag for uniaxialMaterial ConcreteCM" << endln;
		return 0;
	}

	numData = 9;
	if (OPS_GetDoubleInput(&numData, dData) != 0) {
		opserr << "Invalid data for uniaxialMaterial ConcreteCM " << iData[0] << endln;
		return 0;
	}
	...
	if (numArgs == 10) {
		...
		if (strcmp(str, "-GapClose") == 0) {
			if (OPS_GetIntInput(&numData, &gap) != 0) {
				opserr << "Invalid $gap parameter for uniaxialMaterial with tag  " << iData[0] << endln;
				return 0;
			}
		} else {
			opserr << "Invalid input parameter for uniaxialMaterial with tag  " << iData[0] << ", want: -GapClose"<< endln;
			return 0;
		}
