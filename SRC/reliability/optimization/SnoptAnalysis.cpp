...
// ---------------------- step 3 snopt construction for reliability -------------------------------
	...
	this->theConstraintFunction=theReliabilityDomain->getConstraintFunctionPtr(1);
	this->theObjectiveFunction=theReliabilityDomain->getObjectiveFunctionPtr(1);
...
// --- Quan control  from here....	
	this->n     =  theReliabilityDomain->getNumberOfDesignVariables();   // n need to be got from DesignVariables x1 x2 x3 ..
	if (theConstraintFunction ==0) this->neF =1;   // no constraint function
	
	else
		this->neF   =  theConstraintFunction->getNumOfConstraintFunctions()+1;// in this case, only 1  objectivefunction
...
// ------------------ G -----------------  
  ...
  this->lenG   = n*neF; //// for reliability only
...
//--------------------- step 3.2 set x and problem bound for optimization --------------------------------------
  ...
  for ( i=0;i<n;i++){
   theDesignVariable=theReliabilityDomain->getDesignVariablePtr(i+1);
...
int SNOPTAnalysis::runOptAnalysis (ReliabilityDomain *passedReliabilityDomain){
...
//                                                                     |
	if (!needG)  {                                            //       |
	  this->usrfun= toyOptusrf_ ;  // Sets the usrfun that supplies F. |
	  ...
	  this->usrfun= toyOptusrfg_ ; // Sets the usrfun that supplies F,G|
...
// 2. ---- update x
	...
	for(i=0; i<n;i++){
	  ...
	  DesignVariable * theDesignVariable = theReliabilityDomain->getDesignVariablePtr(i+1);
