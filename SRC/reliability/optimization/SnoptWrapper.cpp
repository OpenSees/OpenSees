...
void SnoptWrapper::errMsgExit( char *var )
{
  cerr << "****************************************************\n";
  cerr << "Error: " << var << " must be set prior to call to " << endl
       << "SnoptWrapper::solve() or SnoptWrapper::computeJac()!\n";
  cerr << "****************************************************\n";
  exit(1);
}

void SnoptWrapper::setMemory()
{
  int memoryGuess;
...
void SnoptWrapper::alloc( integer alencw, integer aleniw, integer alenrw )
{
...
void SnoptWrapper::realloc(  integer alencw, integer aleniw, integer alenrw )
{
...
void SnoptWrapper::memcpyIn( char *tcw, integer *tiw, doublereal *trw,
...
void SnoptWrapper::memcpyOut( char *tcw, integer *tiw, doublereal *trw,
			     integer tlencw, integer tleniw, integer tlenrw )
{
  integer mlencw = lencw < tlencw ? lencw : tlencw;
  integer mleniw = leniw < tleniw ? leniw : tleniw;
...
void SnoptWrapper::init()
{
  initCalled = 1;
  sninit_( &iPrint, &iSumm, cw, &lencw, iw, &leniw, rw, &lenrw, 8*500 );
}

void SnoptWrapper::setParameter( char *stropt )
{
  assert( initCalled == 1 );
...
void SnoptWrapper::getParameter( char *stroptin, char *stroptout )
{
  assert( initCalled == 1 );
...
void SnoptWrapper::setIntParameter( char *stropt, integer opt )
{
  assert( initCalled == 1 );
...
void SnoptWrapper::getIntParameter( char *stropt, integer &opt )
{
  assert( initCalled == 1 );
  integer stropt_len = strlen(stropt);
...
void SnoptWrapper::setRealParameter( char *stropt, doublereal opt )
{
  assert( initCalled == 1 );
...
void SnoptWrapper::getRealParameter( char *stropt, doublereal &opt )
{
  assert( initCalled == 1 );
  integer stropt_len = strlen(stropt);
...
void SnoptWrapper::solve( integer starttype )
{
  assert( initCalled == 1 );
...
void SnoptWrapper::setPrintFile(  char aprintname[] )
{
  assert( initCalled = 1 );
  if (iPrint != 0 ) {
    snoptclose_( &iPrint );
...
void SnoptWrapper::setSpecFile( char aspecname[] )
{
  assert( initCalled == 1 );
  if (iSpecs != 0 ) {
    snoptclose_( &iSpecs );
...
void SnoptWrapper::setProblemSize( integer an, integer aneF )
{
  //  checkSet = checkSet+2;
  n   = an;
  neF = aneF;
...
void SnoptWrapper::setObjective( integer aObjRow, doublereal aObjAdd )
{
  //checkSet = checkSet+2;
  ObjRow = aObjRow;
  ObjAdd = aObjAdd;
...
void SnoptWrapper::setA( integer alenA, integer *aiAfun,
...
void SnoptWrapper::setNeA( integer aneA )
{
  //checkSet = checkSet+1;
  neA = aneA;
}
void SnoptWrapper::setG( integer alenG, integer *aiGfun,
			 integer *ajGvar)
{
  //checkSet = checkSet+3;
  lenG  = alenG;
...
void SnoptWrapper::setNeG( integer aneG )
{
  //checkSet = checkSet+1;
  neG = aneG;
}

void SnoptWrapper::setX( doublereal *ax, doublereal *axlow, doublereal *axupp,
...
void SnoptWrapper::setF( doublereal *aF, doublereal *aFlow, doublereal *aFupp,
...
void SnoptWrapper::setXNames( char *axnames, integer anxnames )
{
  //checkSet = checkSet+2;
  xnames  = axnames;
  nxnames = anxnames;
...
void SnoptWrapper::setFNames(char *aFnames, integer anFnames )
{
  //checkSet = checkSet+2;
  Fnames  = aFnames;
  nFnames = anFnames;
...
void SnoptWrapper::setProbName(char *aProb )
{
  //checkSet = checkSet+1;
...
void SnoptWrapper::setUserFun(My_fp ausrfunB )
{
  //checkSet = checkSet+1;
  usrfun = ausrfunB;
}
