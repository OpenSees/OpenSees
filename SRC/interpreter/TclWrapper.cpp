...
static int Tcl_ops_getUniaxialMaterialTags(ClientData clientData, Tcl_Interp *interp, int argc,   TCL_Char **argv) {
    wrapper->resetCommandLine(argc, 1, argv);

    if (OPS_getUniaxialMaterialTags() < 0) return TCL_ERROR;
...
TclWrapper::addOpenSeesCommands(Tcl_Interp* interp)
{
    ...
    addCommand(interp,"getUniaxialMatTags", &Tcl_ops_getUniaxialMaterialTags);
