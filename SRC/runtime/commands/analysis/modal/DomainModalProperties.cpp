...
DomainModalProperties::DomainModalProperties(Domain *domain, bool unorm)
  : m_domain(domain)
  , m_unorm(unorm)
{
}
...
DomainModalProperties::compute()
{
    ...
    */
    ...
    {
        //
        // assemble in the sparse mass matrix
        //
        ...
        auto addM = [&M, &ML, &MLfree, &aux_MD, &aux_ML, &aux_MC, &aux_C, &aux_Lumped](
            const Matrix& iM, const ID& iD, const std::vector<int> &uflags, const std::vector<size_t> &npos) {
            int n = iD.Size();
        ...
        auto addV = [&V](const Matrix& iV, const ID& iD) {
            int n = iD.Size();
    ...
    // Normalize eigenvectors such that their max component is 1
