...
void *OPS_ModElasticBeam3d(const ID &info) {
    // 1. data needed
    int iData[3];
    double data[6];
    int transfTag;
   ...
   connectedExternalNodes(2), theCoordTransf(0), theDamping(0)
{
  ...
  if (E == 0.0) {
    opserr << "ModElasticBeam3d::ModElasticBeam3d - E from section is zero, using E = 1" << endln;
    E = 1.0;
  }
  ...
  if (G == 0.0) {
    opserr << "ModElasticBeam3d::ModElasticBeam3d - G from section is zero, using G = 1" << endln;
    G = 1.0;
  }  
  ...
  if (Jx == 0.0) {
    opserr << "ModElasticBeam3d::ModElasticBeam3d -- no torsion in section -- continuing with GJ = 0\n";
    //Jx = 1.0e10;
  }
