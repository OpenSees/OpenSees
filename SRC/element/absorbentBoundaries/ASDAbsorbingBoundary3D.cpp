...
OPS_ASDAbsorbingBoundary3D(void)
{
    ...
    const char* descr = "Want: element ASDAbsorbingBoundary3D $tag $n1 $n2 $n3 $n4 $G $v $rho $thickness $btype <-fx $tsxTag> <-fy $tsyTag>\n";
    ...
    if (OPS_GetInt(&numData, iData) != 0) {
        opserr << "ASDAbsorbingBoundary3D ERROR: Invalid integer mandatory values: element ASDAbsorbingBoundary3D wants 5 integer parameters\n" << descr;
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr << "ASDAbsorbingBoundary3D ERROR: Invalid double mandatory values: element ASDAbsorbingBoundary3D wants 4 double parameters\n" << descr;
        return 0;
    }
    ...
    if (strcmp(btype, "L") == 0) {
        ...
        opserr << "ASDAbsorbingBoundary3D ERROR: Invalid string mandatory value: the $btype argument should be either "
    ...
    // done
    return new ASDAbsorbingBoundary3D(iData[0], iData[1], iData[2], iData[3], iData[4], dData[0], dData[1], dData[2], dData[3], bflag, fx, fy);
}
...
void ASDAbsorbingBoundary3D::setDomain(Domain* theDomain)
{
    ...
    for (std::size_t i = 0; i < m_nodes.size(); ++i) {
        ...
        if (ndm != 2) {
            opserr << "ASDAbsorbingBoundary3D Error in setDomain: Nodes must have 2 dimensions, not " << ndm << "\n";
            exit(-1);
        }
        ...
        if (ndf < 2) {
            opserr << "ASDAbsorbingBoundary3D Error in setDomain: In 2D at least 2 DOFs are required, not " << ndf << "\n";
            exit(-1);
        }
...
void ASDAbsorbingBoundary3D::getElementSizes(double& lx, double& ly, double& nx)
{
    // get nodes
...
double ASDAbsorbingBoundary3D::penaltyFactor()
{
...
void ASDAbsorbingBoundary3D::getLKcoeff(double& ap, double& as)
{
    // elasticity constants
    double mu = m_G;
    double lam = 2.0 * mu * m_v / (1.0 - 2.0 * m_v);
