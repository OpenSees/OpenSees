...
void BackgroundMesh::addLargeSize(int numbasic, const VDouble& range_low,
...
// -----
// 0   1
void BackgroundMesh::getCorners(const VInt& index, int num, int level,
                                VVInt& indices) const {
    int ndm = OPS_GetNDM();
    int counter = 0;
    int tnum = num * level;
    ...
    while ((mesh = dynamic_cast<Mesh*>(meshes())) != 0) {
        ...
        for (int j = 0; j < group->numParticles(); j++) {
            ...
            if (bcell.type == STRUCTURE) {
                if (level > 1) {
                    opserr << "WARNING: structural cell should have "
                              "finest mesh\n";
                    return -1;
                }
                if (!fsiTri) continue;
    ...
    for (std::map<VInt, BNode>::iterator it = bnodes.begin();
        ...
        for (int i = 0; i < (int)bnode.crdsn.size(); ++i) {
            ...
            if (ndm == 2) {
                ...
                tetgen.addPoint(bnode.crdsn[i][0], bnode.crdsn[i][1],
                                bnode.crdsn[i][2], 0);
            }
        }
    }
...
int BackgroundMesh::convectParticle(Particle* pt, VInt index, int level,
                                    int nums) {
    Domain* domain = OPS_GetDomain();
    if (domain == 0) return 0;
    ...
    while (pt->getDt() > 0) {
        ...
        if (interpolate(pt, indices, vels, incrvels, dvns, pns, dpns, crds,
                        fixed, ndtags, subdt) < 0) {
...
void BackgroundMesh::getWall(VDouble& dir, double& dist,
                             const VDouble& xbnd, const VDouble& ybnd,
                             const VDouble& zbnd, const VDouble pcrds) {
    int ndm = OPS_GetNDM();
