...
OPS_BeamContact3Dp(void)
{
  ...
  if (numRemainingInputArgs < 8) {
    opserr << "Invalid #args,  want: element BeamContact3Dp eleTag?  iNode? jNode? slaveNode? radius? crdTransf? matTag? penalty? <cSwitch>?\n";
    return 0;
  }
...
// constructors:
BeamContact3Dp::BeamContact3Dp(int tag, int Nd1, int Nd2, int NdS, double rad, CrdTransf &coordTransf,
                               NDMaterial &theMat, double pen, int cSwitch)
