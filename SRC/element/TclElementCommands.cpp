...
extern void* OPS_FourNodeQuadWall3d(void); // CSUF
...
// CSUF
extern int
TclModelBuilder_addFourNodeMVLEM3D(ClientData, Tcl_Interp*, int, TCL_Char**,
	Domain*, TclModelBuilder*, int argStart);

// CSUF
extern int
TclModelBuilder_addFourNodeSFI_MVLEM3D(ClientData, Tcl_Interp*, int, TCL_Char**,
	Domain*, TclModelBuilder*, int argStart);
	...
	Domain* theTclDomain, TclModelBuilder* theTclBuilder)
{
	...
	else if ((strcmp(argv[1], "quadwall3d") == 0) || (strcmp(argv[1], "QuadWall3d") == 0)) { // CSUF

	void* theEle = OPS_FourNodeQuadWall3d();
	if (theEle != 0)
		theElement = (Element*)theEle;
	else {
		opserr << "TclElementCommand -- unable to create element of type : " << argv[1] << endln;
	...
	else if (strcmp(argv[1], "FourNodeMVLEM3D") == 0) { // CSUF
		int eleArgStart = 1;
		int result = TclModelBuilder_addFourNodeMVLEM3D(clientData, interp, argc, argv,
			theTclDomain, theTclBuilder, eleArgStart);
		return result;
	}
	else if (strcmp(argv[1], "FourNodeSFI_MVLEM3D") == 0) { // CSUF
		int eleArgStart = 1;
		int result = TclModelBuilder_addFourNodeSFI_MVLEM3D(clientData, interp, argc, argv,
			theTclDomain, theTclBuilder, eleArgStart);
		return result;
	}
