...
int ZeroLengthImplexContact::sendSelf(int commitTag, Channel& theChannel) {
    ...
    ddata(8) = sv.eps(1);
    ddata(9) = sv.eps(2);
    ddata(10) = sv.eps_commit(0);
    ddata(11) = sv.eps_commit(1);
    ddata(12) = sv.eps_commit(2);
    ddata(13) = sv.shear(0);
    ddata(14) = sv.shear(1);
    ddata(15) = sv.shear_commit(0);
...
int ZeroLengthImplexContact::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker) {
    ...
    sv.eps(1) = ddata(8);
    sv.eps(2) = ddata(9);
    sv.eps_commit(0) = ddata(10);
    sv.eps_commit(1) = ddata(11);
    sv.eps_commit(2) = ddata(12);
    sv.shear(0) = ddata(13);
    sv.shear(1) = ddata(14);
    sv.shear_commit(0) = ddata(15);
