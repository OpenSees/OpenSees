   ...
   rho_s(rs), rho_f(rf), kf(kkf), Q(0), Ki(0)
{
    ...
    if (permb_x==0.0 || permb_y==0.0 || permb_z==0.0) {
       opserr<<" Error EightNode_LDBrick_u_p::getDampTensorC123 -- permeability (x/y/z) is zero\n";
       exit(-1);
    }
    ...
    if (kf < 1.0e-6) {
       opserr<<" Error! EightNode_LDBrick_u_p::EightNode_LDBrick_u_p -- zero or almost compression modulus! \n";
       exit(-1);
    }
}
...
void EightNode_LDBrick_u_p::setDomain (Domain *theDomain)
{
  ...
  for (i=0; i <Num_Nodes; i++) {
    ...
    if (theNodes[i] == 0) {
      opserr << "Error: EightNode_LDBrick_u_p: node not found in the domain" << "\n";
      return ;
    }
    ...
    if( Ndof != Num_Dof ) {
      opserr << "Error: EightNode_LDBrick_u_p: has wrong number of DOFs at its nodes " << i << " \n";
      return ;
    }
  }
...
int EightNode_LDBrick_u_p::commitState (void)
{
    ...
    if ((retVal = this->Element::commitState()) != 0) {
      opserr << "EightNode_LDBrick_u_p::commitState () - failed in base class";
      return (-1);
    }
...
int EightNode_LDBrick_u_p::addLoad(ElementalLoad *theLoad, double loadFactor)
{
   ...
   if ( type == LOAD_TAG_BrickSelfWeight ) {
     ...
     *Q = ( this->getForceU() + this->getForceP() )*loadFactor;
     ...
     opserr << "EightNode_LDBrick_u_p::addLoad() " << this->getTag() << ", load type unknown\n";
     return -1;
   }
...
int EightNode_LDBrick_u_p::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  for (i=0; i<Num_Nodes; i++) {
    ...
    if ( RA.Size() != Num_Dof ) {
      opserr << "EightNode_LDBrick_u_p::addInertiaLoadToUnbalance(): matrix and vector sizes are incompatable \n";
      return (-1);
    }
...
const Vector &EightNode_LDBrick_u_p::getResistingForce ()
{
    ...
    for (i=0; i<Num_Nodes; i++) {      
      ...
      if ( disp.Size() != Num_Dof ) {
        opserr << "EightNode_LDBrick_u_p::getResistingForce(): matrix and vector sizes are incompatable \n";
        exit(-1);
      }
      for (j=0; j<Num_Dof; j++) {
...
//=============================================================================
int EightNode_LDBrick_u_p::displaySelf (Renderer &theViewer, int displayMode, float fact)
{
     // Not implemtented yet
     return 0;
}
...
//=============================================================================
Response* EightNode_LDBrick_u_p::setResponse(const char **argv, int argc, Information &eleInfo, OPS_Stream &output)
{
  ...
  output.attr("eleType","Eight_LDNodeBrick_u_p");
...
//=============================================================================
void EightNode_LDBrick_u_p::Print(OPS_Stream &s, int flag)
{
      ...
      for( GP_c_s = 0 ; GP_c_s < Num_IntegrationPts; GP_c_s++ ) {
        ...
        for( GP_c_t = 0 ; GP_c_t < Num_IntegrationPts; GP_c_t++ ) {
          ...
          if ( (theMaterial[where]->setTrialF(ff) ) )
            opserr << "EightNode_LDBrick_u_p::update (tag: " << this->getTag() << "), not converged\n";
        }
      }
    }
...
const Matrix &EightNode_LDBrick_u_p::getStiff (int Ki_flag)
{
    ...
    if (Ki_flag != 0 && Ki_flag != 1) {
      opserr << "Error EightNode_LDBrick_u_p::getStiff() - illegal use\n";
      exit(-1);
    }
    ...
    if (Ki == 0) {
      opserr << "Error EightNode_LDBrick_u_p::getStiff() -";
