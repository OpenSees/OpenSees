...
//Panel shear strain vs shear force
	} else if ((strcmp(argv[0],"Shear")=00) ||
	    (strcmp(argv[0],"shear")=0)){
			int i;
			sprintf(outputData,"e%d",i);
			output.tag("ResponseType",outputData);
  ...
  //              (*(eleInformation.theVector))(13) = trans(6,0)*theMaterial2->getStress();
	...
	case 15:
		if(eleInformation.theVector != 0){
			...
			f2=0
			for (int i=0; i<3; i++){
				f1 +=trans(i,3)*trans(i,1)*trans(i,2)*theMaterial[i]->getStress();
			}
			for (int i=3; i<6; i++){
				f2=+=trans(i,3)*trans(i,1)*trans(i,2)*theMaterial[i]->getStress();
			}
			...
			(*(eleInformation.theVector))(0)=(f1-f2);
			(*(eleInformation.theVector))(i+1)=0.5*(ex2-ex1)*(Lon*Lon+hei*hei)/(Lon*hei);
