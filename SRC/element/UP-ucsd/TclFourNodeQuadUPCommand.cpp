...
// TclModelBuilder_addBBarFourNodeQuadUP(),
// TclModelBuilder_addBBarBrickUP()
//
...
int
TclModelBuilder_addBBarFourNodeQuadUP(ClientData clientData, Tcl_Interp *interp,
				int argc,
				TCL_Char **argv,
				Domain*theTclDomain,
				TclModelBuilder *theTclBuilder)
{
  ...
  BBarFourNodeQuadUP *theBBarFourNodeQuadUP =
      new BBarFourNodeQuadUP(BBarFourNodeQuadUPId,iNode,jNode,kNode,lNode,
		       *theMaterial, "PlaneStrain", thickness, bk, r, perm1, perm2, b1, b2, p);
  if (theBBarFourNodeQuadUP == 0) {
      opserr << "WARNING ran out of memory creating element\n";
      opserr << "BBarFourNodeQuadUP element: " << BBarFourNodeQuadUPId << endln;
      return TCL_ERROR;
  }
...
int
TclModelBuilder_addBBarBrickUP(ClientData clientData, Tcl_Interp *interp,
				int argc,
				TCL_Char **argv,
				Domain*theTclDomain,
				TclModelBuilder *theTclBuilder)
{
  ...
  if ((argc-argStart) < 15) {
    opserr << "WARNING insufficient arguments\n";
    printCommand(argc, argv);
    opserr << "Want: element BBarBrickUP eleTag? N1? N2? N3? N4? N5? N6? N7? N8? matTag? bulk? rhof? perm_x? perm_y? perm_z? <b1? b2? b3?>\n";
    return TCL_ERROR;
  }
