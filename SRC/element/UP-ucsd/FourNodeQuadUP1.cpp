...
// $Source: /usr/local/cvs/OpenSees/SRC/element/UP-ucsd/FourNodeQuadUP1.cpp,v $
...
FourNodeQuadUP::setParameter(const char **argv, int argc, Information &info)
{
        ...
        // quad horizontal permeability
        if (strcmp(argv[0],"hPerm") == 0) {
                info.theType = DoubleType;
                info.theDouble = perm[0];
                return 3;
        }
        // quad horizontal permeability
        if (strcmp(argv[0],"vPerm") == 0) {
                info.theType = DoubleType;
                info.theDouble = perm[1];
                return 4;
        }
...
//NDMaterial*
//FourNodeQuadUP::getMaterialPtr(int GaussPoint){
//return theMaterial[GaussPoint];
//}
