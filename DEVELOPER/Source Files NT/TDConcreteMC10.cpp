	...
	OPS_TDConcreteMC10() {
		...
		// Parse the input line for the material parameters:
			...
			if (numArgs == 18) {
				...
				//ntosic
				theMaterial = new TDConcreteMC10(iData,dData[0],dData[1],dData[2],dData[3],dData[4],dData[5],dData[6],dData[7],dData[8],dData[9],dData[10],dData[11], dData[12], dData[13], dData[14], dData[15], dData[16]);
                if (theMaterial == 0) {
...
TDConcreteMC10::commitState(void)
{
  ...
  //Secant Stiffness for determination of creep strain:
      if (fabs(eps_m/sig)>Ec) { //ntosic: should this be sig/eps_m
          E_i[count+1] = Ec;
      } else {
