...
static int Tcl_ops_damping(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv) {
    wrapper->resetCommandLine(argc, 1, argv);

    if (OPS_Damping() < 0) return TCL_ERROR;
...
TclWrapper::addOpenSeesCommands(Tcl_Interp* interp)
{
    ...
    addCommand(interp,"damping", &Tcl_ops_damping);
