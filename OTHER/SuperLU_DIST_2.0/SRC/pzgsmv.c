...
(
 ...
 SuperMatrix *A_loc,   /* Output. The column indices are translated into
 ...
 )
{
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Enter pzgsmv_init()");
#endif
    ...
    for (i = 0; i < m_loc; ++i) { /* Loop through each row of A */
		...
		if ( p == iam ) { /* Local */
		  ...
		  ind_tosend[ptr_ind_tosend[p]] = jcol;
