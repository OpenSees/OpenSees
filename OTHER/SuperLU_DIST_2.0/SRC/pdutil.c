 ...
 */
int dSolveInit(superlu_options_t_Distributed *options, SuperMatrix *A, 
	       ...
	       SOLVEstruct_t *SOLVEstruct)
{
    ...
    if ( options->IterRefine && options->RefineInitialized == NO ) {
        if ( !(SOLVEstruct->gsmv_comm = (pdgsmv_comm_t *)
	       SUPERLU_MALLOC(sizeof(pdgsmv_comm_t))) )
	    ABORT("Malloc fails for gsmv_comm[]");
	pdgsmv_init(A, row_to_proc, grid, &A_loc, SOLVEstruct->gsmv_comm);
 ...
 */
void dSolveFinalize(superlu_options_t_Distributed *options, SOLVEstruct_t *SOLVEstruct)
{
